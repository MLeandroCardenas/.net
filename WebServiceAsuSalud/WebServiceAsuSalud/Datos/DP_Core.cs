...
namespace Datos
{
    public class DP_Core
    {
        public List<U_seguridad_cliente> traer_datos_cliente(string token)
        {
            ...
            {
                var datos = consulta.clientes.Where(x => x.Token_seguridad == token && x.Fecha_vigencia > DateTime.Now).ToList<U_seguridad_cliente>();
        ...
        public void insertar_token(string token_seguridad, string nombre, string clave)
        {
            using (var conexion = new Mapeo("security"))
            {
                ...
                lista = traer_cliente(nombre, clave);
